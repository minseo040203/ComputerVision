# 과제 02: 마우스를 이용한 페인트 브러시 구현

이 과제는 OpenCV를 사용하여 마우스 이벤트를 처리하고, 사용자가 마우스를 움직이며 그림을 그릴 수 있는 간단한 페인트 브러시 프로그램을 구현하는 실습입니다.

OpenCV의 마우스 이벤트 처리 함수인 `cv2.setMouseCallback()`을 활용하여 마우스를 누르고 움직일 때 화면에 선을 그리도록 구현합니다.

---

# 목표

1. OpenCV 창을 생성하여 이미지를 화면에 표시하기.
2. 마우스 이벤트를 감지하여 그림을 그리는 기능 구현.
3. 마우스를 누르고 이동하면 선이 그려지도록 구현.
4. 키보드 입력(`ESC`)을 이용해 프로그램 종료.

---

# 구현 내용

- OpenCV 창을 생성하고 빈 캔버스를 만든다.
- `cv2.setMouseCallback()`을 사용하여 마우스 이벤트를 감지한다.
- 마우스를 누른 상태에서 이동하면 선을 그리도록 구현한다.
...
```bash
    if event == cv2.EVENT_LBUTTONDOWN:
```
- 마우스 왼쪽 버튼을 눌렀을 때 실행된다.
- 그림을 그리기 시작하는 좌표를 저장한다.

```bash
    cv2.line(img, (ix, iy), (x, y), (255, 0, 0), 5)
```
- 이전 좌표 (ix, iy)에서 현재 좌표 (x, y)까지 선을 그린다.
- (255,0,0)은 파란색(BGR 기준) 색상이다.
- 5는 선의 두께이다.

```bash
    cv2.setMouseCallback('paint', draw)
```
- paint 창에서 발생하는 마우스 이벤트를 draw 함수와 연결한다.
    
```bash
    cv2.imshow('paint', img)
```
- 현재 이미지를 OpenCV 창에 출력한다.
- 사용자가 그린 내용이 실시간으로 화면에 표시된다.

```bash
    if cv2.waitKey(1) & 0xFF == 27:
```
- waitKey()는 키보드 입력을 감지하는 함수이다.
- 27은 ESC 키이다.
- ESC를 누르면 프로그램이 종료된다.
...
3. 아래 명령으로 스크립트 실행
```bash
python 02_paint_brush.py
```

## 결과
- 프로그램 실행 후 빈 캔버스 창이 나타난다.
- 마우스를 클릭하고 이동하면 선이 그려진다.
- ESC 키를 누르면 프로그램이 종료된다.
![alt text](painting_result.jpg)